...
<%
	tDto dto = (tDto)request.getAttribute("tradepostre"); // 한줄씩 모든 정보를 들고 오고싶으면 vector를 사용 한줄에 대한 각 칼럼 별로 정보를 들고오고 싶으면 dto에 담아서 가져온다
	int brx=dto.getBrx();
   ...
   <div id="head">
	...
	<div id="wrap">
        ...
        function del() {        	
         	if(idchk==writer)
         	{
        		location.href = location.href = '<%=request.getContextPath()%>/Controller?cmd=delete&brx=<%=dto.getBrx()%>&category=trade';
        	}
        	else
        	{
        		alert("작성자가 아닙니다");
        ...
        {         	
        	if(idchk==writer)
        	{
        		location.href = '<%=request.getContextPath()%>/Controller?brx=<%=dto.getBrx()%>&cmd=selectOne_trade';
        	}
        	else
        	{
        		alert("작성자가 아닙니다.");
        ...
        <div id="board">
			...
			<%
				...
				<tr>
					<th><a
						href="<%=request.getContextPath()%>/asdController?cmd=tradepost&brx=<%=trade.get(i).getBrx()%>">
							<%
		...
		<a href="<%=request.getContextPath()%>/asdController?cmd=trade&cat=trade&pageNum=<%= i %>">[<%= i %>]</a> 
	<%} %>
        </div>
