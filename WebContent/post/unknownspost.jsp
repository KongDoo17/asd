...
<%
	tDto dto = (tDto)request.getAttribute("unknownspostre"); // 한줄씩 모든 정보를 들고 오고싶으면 vector를 사용 한줄에 대한 각 칼럼 별로 정보를 들고오고 싶으면 dto에 담아서 가져온다
	int brx=dto.getBrx();
   ...
   <div id="head">
	...
	<div id="wrap">
        <div id="titlebar">
            ...
            <input type="text" id="writer" name="" value="익명"  readonly>
            <input type="hidden" id="writers" name="" value="<%=dto.getWriter()%>"  readonly>
        ...
        <input type = "button" id="upup" value="추천" onclick = "location.href = 'likeaction.jsp?brx=<%=brx%>&good=<%=good+1%>&cmd=trade'">
    	...
    	var writer = document.getElementById('writers').value;
        function del() {        	
         	if(idchk==writer)
         	{
        		location.href = location.href = '<%=request.getContextPath()%>/Controller?cmd=delete&brx=<%=dto.getBrx()%>&category=unknowns';
        	}
        	else
        	{
        		alert("작성자가 아닙니다");
        ...
        {         	
        	if(idchk==writer)
        	{
        		location.href = '<%=request.getContextPath()%>/Controller?brx=<%=dto.getBrx()%>&cmd=selectOne_unknowns';
        	}
        	else
        	{
        		alert("작성자가 아닙니다.");
        ...
        <div id="board">
			...
			<%
					Vector<tDto> unknowns = (Vector<tDto>) request.getAttribute("unknownsre");
				...
				<tr>
					<th><a
						href="<%=request.getContextPath()%>/asdController?cmd=unknownspost&brx=<%=unknowns.get(i).getBrx()%>">
							<%
		...
		<a href="<%=request.getContextPath()%>/asdController?cmd=unknown&cat=unknowns&pageNum=<%= i %>">[<%= i %>]</a> 
	<%} %>
        </div>
